...
Make LLMs robust to variable token granularity. Insert a **SpanEncoder** that merges nearby token embeddings into span representations, reducing sequence length. Train a **BoundaryPredictor** via hybrid RL to decide which tokens to merge.

**Reproducibility**: Seed=42 fixed everywhere. Git commit `d4fa4a9`. All runs tracked on [wandb](https://wandb.ai/highattacker/adaptive-tokenization) with group `exp-{timestamp}-g{commit}`. Checkpoints saved to Modal volume `adaptive-tok-v2` at step intervals.
...
| Training slowed 40x at step ~1000 | `tokenizer.decode()` called per token position in Python loop; nested Python loops for span grouping | Remove tokenizer calls; vectorize with direct indexing |
...
### Experiment 2: 100M OpenOrca tokens (Successful — Final Run)
...
| Seed | 42 |
| Volume | adaptive-tok-v2 |

**V1 (Naive)**: Standard fine-tuning, loss on answer tokens only. 6000 steps, ~2 it/s.
**V2 (Adaptive)**: SpanEncoder with curriculum. 6000 steps, ~2 it/s. Slower than V1 due to SpanEncoder overhead.
...
- **Adaptive is compression-robust**: 6.5% degradation vs 24.5% for Naive
- **Minimal quality tradeoff**: Without compression, Adaptive is ~5% worse than Naive (1.53 vs 1.46)
- **Curriculum works**: Starting at 0% compression lets SpanEncoder warm up before merging
- **Wandb curves**: Loss drops smoothly for both models. `cr_actual` converges toward `cr_schedule` as curriculum progresses. See wandb run `exp-*-phase1-adapt`.

### Oracle Analysis

Brute-force sampled configurations for single prompts from held-out dataset to find the loss-compression Pareto frontier.

**96-token prompt**: Best configuration at cr=67% achieves loss=4.96 — beating naive no-merge (5.26) by 5.8%.

**384-token prompt**: Best configuration at cr=65% achieves loss=4.04 — beating naive no-merge (4.51) by 10.3%. Green zone (compression improves quality) extends to cr≈65-70%.
...
**Attempt 4: Mild biases [-0.5, -0.15, 0.15, 0.5]**: Small perturbations meant to preserve natural output while adding diversity. Used for Stage 3 GRPO.
...
**Result (GRPO-only, biased, 1000 steps)**:

| Method | Compression | Loss | CORE |
|--------|------------|------|------|
| No-merge baseline | 0% | 1.363 | 1.000 |
| GRPO-only predictor | 75% | 1.528 | 1.121 |
...
### Hybrid Phase 2 Results (Final Run)
...
**CORE < 1.0**: The GRPO fine-tuned predictor achieves *better* quality than no compression while using 66% fewer prompt tokens. Compression improves predictions — the SpanEncoder + BoundaryPredictor together act as a learned noise filter.

**BCE-only vs GRPO**: The imitation-only predictor is CORE=1.036 (worse than no-merge — it compresses but loses quality). After 2000 GRPO steps, it becomes CORE=0.991 (compression improves quality). GRPO is essential — it teaches the predictor to be more aggressive (60% → 66% cr) while simultaneously improving prediction quality.

**Merge visualization**: Wandb artifacts `merge_bce_samples` and `merge_grpo_samples` contain side-by-side tables of how each predictor merges 10 example prompts. Inspectable in the wandb run dashboard.
...
- **Stage 3 unlocks improvement**: Starting from good layouts, the predictor maintains enough variance across samples for the RL advantage signal to be meaningful. GRPO then teaches it to be more aggressive while improving quality.
...
- `modal.Secret.from_dict()` provides API keys to containers without hardcoding
...
- Checkpoint every 1000 steps for Phase 1 (6 per run) and every 500 steps for Phase 2 GRPO (4 per run)

### Experiment Tracking
- Wandb runs grouped by experiment ID (`exp-{timestamp}-g{commit}`)
- Each stage gets its own wandb run with monotonically increasing steps
- Wandb Tables for merge visualization (BCE vs GRPO token groupings)
- Config dict includes seed, model, batch, LR, curriculum params, volume name
...
- `max_span_len=4` gives average 2.5 tokens per span → natural ~60% compression floor
...
- **GRPO > BCE**: imitation alone is CORE=1.036; GRPO fine-tuning flips it to CORE=0.991
